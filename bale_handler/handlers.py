...
from bale_handler.formatters import HtmlFormatter
...
class TelegramHandler(logging.Handler):
    API_ENDPOINT = 'https://tapi.bale.ai'
    last_response = None
    ...
    def emit(self, record: LogRecord):
        try:
            ...
            for t in traceback.format_tb(record.exc_info[2]):
                text += "\n{}".format(t)

        except Exception as e:
            text = "Exception in generating report message:\n{}\n\ndata:\n{}\n{}".format(
                e,
                record.getMessage(),
                record.exc_info
            )
