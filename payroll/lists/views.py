...
from payroll.lists.filters import WorkshopFilter, PersonnelFilter, PersonnelFamilyFilter, WorkshopPersonnelFilter, \
    ContractRowFilter, ContractTimeFilter
...
from payroll.serializers import WorkShopSerializer, PersonnelSerializer, PersonnelFamilySerializer, \
    WorkshopPersonnelSerializer, ContractRowSerializer, ContractTimeSerializer
...
class ContractTimeListView(generics.ListAPIView):
    permission_classes = (IsAuthenticated, BasicCRUDPermission)

    permission_codename = "get.contract_time"
    serializer_class = ContractTimeSerializer
    filterset_class = ContractTimeFilter
    pagination_class = LimitOffsetPagination

    def get_queryset(self):
        return ContractTime.objects.hasAccess('get', self.permission_codename).all()
