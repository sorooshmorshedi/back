...
def write_tax_diskette(request, pk):
    ...
    for item in items:
        if item.is_month_tax:
            ...
            content += str(1)
            content += ','
            content += str(round(item.tax_year_real_work_month))
            content += ','
            content += str(0)
            content += ','
            content += str(85)
            ...
            content += str(1)
            content += ','
            content += str(item.contract.tax_add_date.__str__().replace('-', ''))
            content += ','
            content += '' #end work date
            content += ','
            ...
            content += str(round(item.get_hagh_sanavat_and_save_leaves))
            content += ','
            content += str(round(item.manategh_tejari_moafiat))
            content += ','
            content += str(round(item.ejtenab_maliat_mozaaf))
            content += ','
            content += str(str(round(item.total_tax)))
            content += ','
            content += str(str(round(item.total_tax)))
            content += ','
            content += '\n'
...
def write_summary_tax_diskette(request, pk):
    ...
    content += '0'
    content += ','
    content += str(item.sign_date[0].__str__().replace('-', ''))
    content += ','
    content += str(6)
    content += ','
    content += ''
...
def write_new_persons_diskette(request, pks):
    ...
    for workshop_personnel in workshop_personnels:
        ...
        content += ''  #country
        content += ','
        content += '' #personnel_code
        content += ','
        content += str(workshop_personnel.personnel.degree_education)
        content += ','
        content += workshop_personnel.title.name
        content += ','
        content += str(workshop_personnel.personnel.insurance_for_tax)
        content += ',,'
        content += str(workshop_personnel.personnel.insurance_code_for_tax)
        content += ','
        content += str(workshop_personnel.personnel.postal_code)
        content += ','
        content += '' # address
        content += ','
        content += str(workshop_personnel.employment_date.__str__().replace('-', ''))
        content += ','
        content += str(workshop_personnel.employment_type)
        content += ','
        content += str(workshop_personnel.job_location)
